...
如果需要最小、无依赖的 Windows 客户端，请使用
[YacgoSomaz/CatchMe-Lite](https://github.com/YacgoSomaz/CatchMe-Lite)。它是约 50 KB
的 PowerShell/.NET 源码客户端，不包含 Python，也不会在启动后下载运行依赖；双击名称明确的
“启动并授权”入口即可一次完成授权、启动托盘和配置登录启动项。
